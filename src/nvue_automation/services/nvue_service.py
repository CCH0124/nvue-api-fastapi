...
from nvue_automation.models.schemas import ApplyOptions, StateControls
...
class AsyncNVUEService:
    ...
    async def apply_changeset(
        ...
        logger.debug(f"[APPLY] NVUE Request payload: {apply_payload}")
    ...
    async def wait_for_applied(self, changeset: str) -> dict[str, Any]:
        ...
        for i in range(self.settings.retries):
            ...
            if state == RevisionState.APPLIED_AND_SAVED.value:
                logger.info(
                    f"[POLL] Configuration successfully applied and saved | "
                    f"changeset={changeset} | final_state={state}"
                )
                return resp
    ...
    async def config_replace(
        ...
        try:
            ...
            if options.should_wait_for_applied():
                ...
                logger.info(
                    f"[REPLACE] ⚠️  Skipping wait (manual confirmation required or disabled) | changeset={changeset}"
                )
            ...
            revision_info["_changeset"] = changeset
    ...
    async def config_merge(
        ...
        try:
            ...
            if options.should_wait_for_applied():
                ...
                logger.info(
                    f"[MERGE] Skipping wait (manual confirmation required or disabled) | changeset={changeset}"
                )
                revision_info = await self.config_history(changeset)
    ...
    async def config_save(self, changeset: str, message: str | None = None) -> dict[str, Any]:
        """
        ...
        """
        logger.info(f"[SAVE] Initiating save operation | changeset={changeset}")
        revision_info = await self.config_history(changeset) 
        current_state = revision_info.get("state")
        if current_state == RevisionState.APPLIED_AND_SAVED.value or current_state == RevisionState.CONFIRM_FAIL.value or current_state == RevisionState.DETACHED.value:
            logger.warning(
                f"[SAVE] Changeset already saved | changeset={changeset} | "
                f"state={current_state} | action=skip"
            )
            return revision_info
        logger.info(f"[SAVE] Persisting configuration to startup | changeset={changeset} | current_state={current_state}")
        options = ApplyOptions()
        options.auto_prompt = AutoPrompt(ays="ays_yes", confirm="confirm_yes")
        if message:
            options.message = message
            logger.debug(f"[SAVE] Using custom message: '{message}'")
        options_payload = options.to_payload()

        # Build base payload
        apply_payload: dict[str, Any] = {"state": RevisionState.CONFIRM_YES.value}
        apply_payload.update(options_payload)

        logger.debug(f"[SAVE] Payload prepared: {apply_payload}")
